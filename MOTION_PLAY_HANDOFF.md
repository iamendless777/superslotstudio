...
1. Smoke Play Motion: cluster-hex, classic-nine, sticky-five, anticipation-five on a loaded project.
2. If pixels are wrong, inspect `playStakeTumble` / reel tracks — do not revive ad-hoc WAAPI.
3. Art loop can run in parallel: `npm run studio -- art-brief <template>`.
4. Optional: merge `integrate/studio-motion` → `main` after smoke.
...
**Play Motion: cluster → playStakeTumble; reel templates → Preview spin tracks. Fixtures are generated by `npm run studio -- cues --all`. Smoke the four templates; then ship art. Do not invent a second animator.**
