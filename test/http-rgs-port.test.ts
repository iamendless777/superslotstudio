...
test("rejects duplicate security parameters and hosts outside an allowlist", () => {
  for (const url of [
    "https://game.example/?sessionID=first&sessionID=second&rgs_url=rgs.example",
    "https://game.example/?sessionID=s&rgs_url=rgs.example&rgs_url=evil.example",
    "https://game.example/?sessionID=s&rgs_url=rgs.example&lang=en&lang=fr",
    "https://game.example/?sessionID=s&rgs_url=rgs.example&device=desktop&device=mobile",
  ]) {
    assert.throws(
      () => parseLaunchConfiguration(url),
      InvalidLaunchConfigurationError,
  ...
  const url = "https://game.example/?sessionID=s&rgs_url=rgs.example:443";
  assert.equal(
    parseLaunchConfiguration(url, { allowedRgsHosts: ["RGS.EXAMPLE:443"] })
      .rgsBaseUrl.href,
    "https://rgs.example/",
  );
  assert.throws(
    () =>
      parseLaunchConfiguration(url, { allowedRgsHosts: ["trusted.example"] }),
    InvalidLaunchConfigurationError,
  );
});
