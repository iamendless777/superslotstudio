...
## Milestone 12 — deterministic recovery evidence

Status: **local simulation complete; authorized staging verification pending**.

- [x] Simulate Play interruption before and after authoritative commit.
- [x] Simulate committed checkpoint and EndRound response interruption.
- [x] Recreate a client after checkpoint uncertainty and authenticate authoritative
      progress.
- [x] Assert each ambiguous monetary mutation is dispatched exactly once.
- [x] Emit a versioned, deterministic, JSON-safe evidence report.
- [ ] Repeat the response-path scenarios only in an explicitly authorized Stake
      Engine development or staging session.
